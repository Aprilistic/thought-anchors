...
def find_best_matches_fully_batched(
    target_step: str, all_rollout_steps: List[List[str]], model, threshold: float = 0.7
    ...
    for i, (rollout_idx, step_idx, similarity) in enumerate(
        zip(rollout_indices, step_indices, all_similarities)
    ):
        if similarity >= threshold and (
            best_matches[rollout_idx][0] == -1
            or similarity > best_matches[rollout_idx][1]
        ):
...
def process_chunk_pair(args):
    ...
    include_i_rollouts = [
        r for r in include_i_rollouts if "rollout" in r and r["rollout"]
    ]
    exclude_i_rollouts = [
        r for r in exclude_i_rollouts if "rollout" in r and r["rollout"]
    ]
    ...
    include_i_steps = [
        extract_steps_from_rollout(rollout["rollout"]) for rollout in include_i_rollouts
    ]
    exclude_i_steps = [
        extract_steps_from_rollout(rollout["rollout"]) for rollout in exclude_i_rollouts
    ]

    # Process all rollouts where step i was kept
    include_i_matches = find_best_matches_fully_batched(
        target_step, include_i_steps, embedding_model, similarity_threshold
    )

    # Process all rollouts where step i was removed
    exclude_i_matches = find_best_matches_fully_batched(
        target_step, exclude_i_steps, embedding_model, similarity_threshold
    )
    ...
    include_i_match_rate = (
        len(include_positives) / len(include_i_matches) if include_i_matches else 0
    )
    exclude_i_match_rate = (
        len(exclude_positives) / len(exclude_i_matches) if exclude_i_matches else 0
    )

    # Calculate importance as difference in match rate and similarity
    importance = (
        include_i_match_rate - exclude_i_match_rate
    )  # NOTE: For now, we only use match rate difference
    """
...
def compute_step_importance_matrix(
    ...
    for chunk_idx in range(num_chunks):
        ...
        all_rollouts[chunk_idx] = [
            r for r in rollouts if "rollout" in r and r["rollout"]
        ]
    ...
    for chunk_idx, rollouts in tqdm(
        all_rollouts.items(), desc="Pre-computing rollout steps and embeddings"
    ):
    ...
    with tqdm(
        total=num_chunks * (num_chunks - 1) // 2, desc="Processing chunk pairs"
    ) as pbar:
        for i in range(num_chunks - 1):
            ...
            if (
                include_i_idx not in chunk_rollout_steps
                or exclude_i_idx not in chunk_rollout_steps
            ):
                pbar.update(len(j_values))
                continue

            if (
                len(chunk_rollout_steps[include_i_idx]["steps"]) == 0
                or len(chunk_rollout_steps[exclude_i_idx]["steps"]) == 0
            ):
                pbar.update(len(j_values))
                continue
            ...
            for j in j_values:
                ...
                include_similarities = cosine_similarity(
                    target_embedding, include_embeddings
                )[0]
                include_matches = [(-1, 0.0)] * include_steps_info["num_rollouts"]

                # Find best match for each rollout where chunk i was kept
                for step_idx, (rollout_idx, similarity) in enumerate(
                    zip(include_steps_info["rollout_indices"], include_similarities)
                ):
                    if similarity >= similarity_threshold and (
                        include_matches[rollout_idx][0] == -1
                        or similarity > include_matches[rollout_idx][1]
                    ):
                        include_matches[rollout_idx] = (
                            include_steps_info["step_indices"][step_idx],
                            float(similarity),
                        )

                # Process exclude_i rollouts (chunk i was removed)
                exclude_similarities = cosine_similarity(
                    target_embedding, exclude_embeddings
                )[0]
                exclude_matches = [(-1, 0.0)] * exclude_steps_info["num_rollouts"]

                # Find best match for each rollout where chunk i was removed
                for step_idx, (rollout_idx, similarity) in enumerate(
                    zip(exclude_steps_info["rollout_indices"], exclude_similarities)
                ):
                    if similarity >= similarity_threshold and (
                        exclude_matches[rollout_idx][0] == -1
                        or similarity > exclude_matches[rollout_idx][1]
                    ):
                        exclude_matches[rollout_idx] = (
                            exclude_steps_info["step_indices"][step_idx],
                            float(similarity),
                        )
                ...
                include_match_rate = (
                    len(include_positives) / len(include_matches)
                    if include_matches
                    else 0
                )
                exclude_match_rate = (
                    len(exclude_positives) / len(exclude_matches)
                    if exclude_matches
                    else 0
                )
...
def plot_step_importance_matrix(
    ...
    ax = sns.heatmap(
        ...
        norm=mcolors.Normalize(
            vmin=-max(abs(plot_matrix.min()), abs(plot_matrix.max())),
            vmax=max(abs(plot_matrix.min()), abs(plot_matrix.max())),
        ),
    ...
    for i in range(len(chunk_texts)):
        for j in range(len(chunk_texts)):
            if j <= i:  # Upper triangle (including diagonal)
                ...
                ax.add_patch(
                    plt.Rectangle((i, j), 1, 1, fill=True, color="#FFFFFF", zorder=10)
                )
...
def analyze_step_attribution(
    ...
    for problem_dir in tqdm(problem_dirs, desc="Analyzing problems"):
        ...
        is_correct = "correct_base_solution" in str(
            problem_dir
        ) and "incorrect_base_solution" not in str(problem_dir)
        ...
        if base_solution:
            with open(
                problem_output_dir / "base_solution.json", "w", encoding="utf-8"
            ) as f:
        ...
        if num_top_sentences is not None:
            ...
            print(
                f"Selected top {len(chunk_texts)} sentences for problem {problem_idx}"
            )

        # Filter chunks based on excluded abbreviations (after top selection)
        if excluded_abbreviations:
            filtered_chunk_texts, kept_indices = filter_chunks_by_excluded_tags(
                chunk_texts, excluded_abbreviations
            )

            # Filter the importance matrix to keep only the rows and columns for kept chunks
            if kept_indices:
                importance_matrix = importance_matrix[
                    np.ix_(kept_indices, kept_indices)
                ]
                ...
                print(
                    f"Filtered to {len(chunk_texts)} chunks after excluding abbreviations: {excluded_abbreviations}"
                )
            else:
                print(
                    f"Warning: All chunks excluded for problem {problem_idx}, skipping plots"
                )
                continue
        ...
        for i, chunk in enumerate(chunk_texts):
            ...
            color = (
                CATEGORY_COLORS.get(category, "#000000") if category else "#000000"
            )  # Default to black
        ...
        for i in range(len(chunk_texts)):
            for j in range(len(chunk_texts)):
                if j <= i:  # Upper triangle (including diagonal)
                    ...
                    ax.add_patch(
                        plt.Rectangle(
                            (i, j), 1, 1, fill=True, color="#FFFFFF", zorder=10
                        )
                    )
        ...
        for i in range(len(chunk_texts)):
            ...
            outgoing_importance[i] = (
                np.mean(future_steps) if len(future_steps) > 0 else 0
            )
        ...
        for j in range(len(chunk_texts)):
            ...
            incoming_importance[j] = (
                np.mean(previous_steps) if len(previous_steps) > 0 else 0
            )
        ...
        for i in range(len(chunk_texts)):
            ...
            step_data = {
                "source_chunk_idx": i,
                "source_chunk_text": chunk_texts[i].get("chunk", str(chunk_texts[i]))
                if isinstance(chunk_texts[i], dict)
                else str(chunk_texts[i]),
        ...
        # Save to JSON file
        with open(
            problem_output_dir / "step_importance.json", "w", encoding="utf-8"
        ) as f:
        ...
        summary["top_dependent_steps"] = [
            ...
            for idx in top_dependent_indices
            if incoming_importance[idx] > 0
        ]
...
def get_problem_dirs(
    ...
    if include_problems:
        all_problem_dirs = [
            ...
            if int(d.name.split("_")[-1])
            in [int(p) for p in include_problems.split(",")]
        ]
...
def select_top_sentences(importance_matrix, chunk_texts, num_top_sentences):
    ...
    combined_scores, outgoing_scores, incoming_scores = (
        calculate_sentence_importance_scores(importance_matrix)
    )

    # Get indices of top N sentences
    top_indices = np.argsort(combined_scores)[-num_top_sentences:][
        ::-1
    ]  # Descending order
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze step-to-step attribution in chain-of-thought reasoning"
    )
    parser.add_argument(
        "-ad",
        "--analysis_dir",
    ...
    parser.add_argument(
        ...
        "--output_dir",
        type=str,
        default="analysis/step_attribution",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.8,
        help="Threshold for considering steps similar",
    )
    parser.add_argument(
        "-mc",
        "--max_chunks",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated list of problem indices to include",
    )
    parser.add_argument(
        "-co",
        "--correct_only",
        action="store_true",
        help="Only analyze correct solutions, if not specified, only incorrect solutions will be analyzed",
    )
    parser.add_argument(
        "-nc",
        "--no_cache",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated list of function tag abbreviations to exclude (e.g., 'SC,PS')",
    )
    parser.add_argument(
        "-nt",
        "--num_top_sentences",
        type=int,
        default=None,
        help="Number of top sentences to show in importance matrix (None = show all)",
    )
