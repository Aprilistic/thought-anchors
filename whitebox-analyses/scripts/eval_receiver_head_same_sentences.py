...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate receiver head reliability using split-half correlation"
    )
    ...
    parser.add_argument(
        "--marker-size", type=int, default=20, help="Size of scatter points"
    )
    parser.add_argument(
        "--output-dir", type=str, default="plots/kurt_plots", help="Output directory"
    )
    parser.add_argument("--dpi", type=int, default=300, help="DPI for saved figure")
    parser.add_argument(
        "--xlim", type=float, nargs=2, default=[-1, 42], help="X-axis limits"
    )
    parser.add_argument(
        "--ylim", type=float, nargs=2, default=[-1, 42], help="Y-axis limits"
    )
    parser.add_argument(
        "--tick-interval", type=int, default=10, help="Interval between ticks"
    )
    parser.add_argument(
        "--top-k", type=int, default=32, help="Top-k for the receiver heads"
    )
    ...
    top_k_layer_head = top_k_layer_head[
        (top_k_layer_head[:, 0] < n_layers) & (top_k_layer_head[:, 1] < n_heads)
    ]
    ...
    num_layers, num_heads = n_layers, n_heads
