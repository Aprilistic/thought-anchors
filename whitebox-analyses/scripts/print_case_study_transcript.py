...
def print_transcript(args):
    ...
    fp_in = (
        Path(args.base_dir)
        / args.model_name
        / args.temperature_folder
        / args.solution_type
        / f"problem_{args.problem_num}"
        / args.input_file
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Print case study transcript in LaTeX or plain text format"
    )

    # File path configuration
    parser.add_argument(
        "--base-dir", type=str, default="rollouts", help="Base directory for rollouts"
    )
    parser.add_argument(
        "--model-name",
        type=str,
