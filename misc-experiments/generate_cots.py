...
def initialize_tokenizer(model_name):
    ...
    try:
        ...
        hf_model = hf_model_map.get(
            base_model, f"deepseek-ai/DeepSeek-R1-Distill-Qwen-14B"
        )
...
async def make_openrouter_request(
    ...
    """Make a direct HTTP request to OpenRouter or an OpenAI-compatible vLLM server."""
...
async def generate_solutions_parallel(
    problems: List[Tuple[int, Dict]], args
) -> List[Dict]:
    """
    ...
    if args.logit_bias_tokens and args.logit_bias_strength is not None:
        output_dir = (
            output_dir
            / f"temperature_{str(args.temperature)}_top_p_{str(args.top_p)}_logit_bias_{args.logit_bias_tokens}_{args.logit_bias_strength}"
        )
    else:
        output_dir = (
            output_dir / f"temperature_{str(args.temperature)}_top_p_{str(args.top_p)}"
        )
    ...
    if existing_solutions and not args.skip_recalculate:
        print(
            f"Recalculating accuracy for {len(existing_solutions)} existing solutions..."
        )
        updated_count = 0
        ...
        for sol in existing_solutions:
            ...
            if "error" in sol:
                solutions_to_retry.append(
                    (sol.get("problem_idx", -1), sol.get("run_id", 0))
                )
                continue
    ...
    if solutions_to_retry:
        print(
            f"Found {len(solutions_to_retry)} solutions with errors or empty answers to retry"
        )
    ...
    for i, (problem_idx, problem) in enumerate(problems):
        ...
        for run_id in range(args.runs_per_problem):
            ...
            if (
                problem_idx in existing_solutions_dict
                and run_id in existing_solutions_dict[problem_idx]
                and (problem_idx, run_id) not in solutions_to_retry
            ):
                continue
            ...
            if args.logit_bias_tokens and args.logit_bias_strength is not None:
                ...
                logit_bias = create_logit_bias(
                    args.logit_bias_tokens, args.logit_bias_strength
                )
    ...
    print(
        f"Generating {len(tasks)} new solutions with max concurrency of {args.concurrency}..."
    )
    new_solutions = []
...
def analyze_results(solutions: List[Dict]) -> Dict:
    ...
    for problem_idx, sols in problem_results.items():
        ...
        incorrect = sum(
            1
            for sol in sols
            if not sol.get("is_correct", False)
            and sol.get("answer", "")
            and "error" not in sol
        )
...
async def main():
    ...
    parser.add_argument(
        ...
        "--top_k",
        type=int,
        default=ENDPOINTS.default_top_k,
    ...
    parser.add_argument(
        "-ty", "--type", type=str, default=None, help="Problem type filter"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated tokens to apply logit bias to",
    )
    parser.add_argument(
        "-lbs",
        "--logit_bias_strength",
        type=int,
        default=None,
        help="Strength of logit bias (-100 to 100)",
    )
    parser.add_argument(
        "-c",
        "--concurrency",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Skip recalculating accuracy for existing solutions",
    )
    ...
    for i, (problem_idx, metrics) in enumerate(hardest_problems[:10]):
        ...
        print(
            f"   Accuracy: {metrics['accuracy'] * 100:.2f}% ({metrics['correct']}/{metrics['total_runs']} correct)"
        )
