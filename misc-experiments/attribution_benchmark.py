...
parser.add_argument(
    ...
    "--analysis_dir",
    type=str,
    default="math_rollouts/deepseek-r1-distill-qwen-14b/temperature_0.6_top_p_0.95",
...
parser.add_argument(
    ...
    "--output_dir",
    type=str,
    default="analysis/attribution_benchmark",
...
parser.add_argument(
    "-b", "--base_model", type=str, default="Qwen/Qwen2.5-14B", help="Base model name"
)
parser.add_argument(
    "-mp",
    "--max_problems",
...
parser.add_argument(
    "-l", "--layer", type=int, default=47, help="Layer to analyze for KL attribution"
)
parser.add_argument(
    "-co",
    "--correct_only",
    action="store_true",
    default=True,
    help="Only analyze correct solutions",
)
parser.add_argument(
    "-uai",
    "--use_abs_importance",
    action="store_true",
    default=True,
    help="Use absolute importance values",
)
parser.add_argument(
    "-it",
    ...
    type=float,
    default=None,
    help="Importance threshold for chunks",
)
parser.add_argument(
    "-tm",
    "--target_module",
    ...
    help="Which module to use for KL attribution (attention, mlp, or all)",
)
        ...
        return Pipeline(
            [
                ...
                (
                    "regressor",
                    ElasticNet(
                        alpha=0.002, l1_ratio=0.9, max_iter=10000, random_state=42
                    ),
                ),
            ]
        )
...
def get_problem_dirs(
    analysis_dir: Path, correct_only: bool = True, limit: Optional[int] = None
) -> List[Path]:
    """
    ...
    all_problem_dirs = [
        d for d in base_dir.iterdir() if d.is_dir() and d.name.startswith("problem_")
    ]
    ...
    print(
        f"  - {complete_count} ({complete_count / total_problems * 100:.1f}%) have complete chunk folders"
    )
    print(
        f"  - {incomplete_count} ({incomplete_count / total_problems * 100:.1f}%) have incomplete chunk folders"
    )
    print(
        f"  - {no_chunks_count} ({no_chunks_count / total_problems * 100:.1f}%) have no chunks data"
    )
...
def load_labeled_chunks(problem_dir: Path) -> List[Dict]:
    ...
    for chunk in chunks_data:
        ...
        if args.importance_threshold:
            chunk["importance"] = (
                chunk["importance"]
                if chunk["importance"] >= args.importance_threshold
                else 0
            )
...
def llm_attribution(
    problem: Dict, chunks: List[Dict], base_solution: Dict, **kwargs
) -> List[float]:
    """
    ...
    try:
        ...
        try:
            ...
            if len(ordered_indices) != len(chunk_texts):
                print(
                    f"Warning: Got {len(ordered_indices)} indices but expected {len(chunk_texts)}."
                )

                # If we have too few, append missing indices in arbitrary order
                if len(ordered_indices) < len(chunk_texts):
                    missing_indices = set(range(len(chunk_texts))) - set(
                        ordered_indices
                    )
...
def kl_attribution(
    problem: Dict, labeled_chunks: List[Dict], base_solution: Dict, **kwargs
) -> List[float]:
    """
    ...
    try:
        ...
        with torch.no_grad():
            thinking_outputs = thinking_model(
                input_ids=input_ids, attention_mask=attention_mask
            )
        ...
        for pos in range(input_ids.shape[1] - 1):  # Exclude the last token
            ...
            base_log_probs = torch.nn.functional.log_softmax(
                base_next_token_logits, dim=-1
            )
            thinking_log_probs = torch.nn.functional.log_softmax(
                thinking_next_token_logits, dim=-1
            )

            kl_div = (
                (base_probs * (base_log_probs - thinking_log_probs)).sum(dim=-1).item()
            )
        ...
        if len(attribution_scores) < len(labeled_chunks):
            attribution_scores.extend(
                [0.0] * (len(labeled_chunks) - len(attribution_scores))
            )

        # Replace NaN values with zeros
        attribution_scores = [
            0.0 if np.isnan(score) else score for score in attribution_scores
        ]
...
def perplexity_attribution(
    ...
    try:
        ...
        attribution_scores = [
            0.0 if np.isnan(score) else (1000.0 if np.isinf(score) else score)
...
def evaluate_attribution_method(
    problem_dirs: List[Path], attribution_method: Callable, method_name: str, **kwargs
) -> Dict:
    """
    ...
    for problem_dir in tqdm(problem_dirs, desc=f"Evaluating {method_name}"):
        ...
        true_scores = [
            np.abs(chunk.get("importance", 0.0))
            if kwargs.get("use_abs_importance")
            else chunk.get("importance", 0.0)
        ...
        predicted_scores = attribution_method(
            problem, labeled_chunks, base_solution, **kwargs
        )
        ...
        if predicted_scores:
            ...
            if max_pred > min_pred:
                predicted_scores = [
                    (s - min_pred) / (max_pred - min_pred) for s in predicted_scores
                ]
    ...
    **kwargs,
    ...
    plot_kwargs = {
        ...
        not in [
            "activation_model",
            "metrics_model",
            "activation_position_model",
            "metrics_position_model",
            "position_model",
        ]
    }
    ...
    for problem_dir in tqdm(problem_dirs, desc="Plotting attribution metrics"):
        ...
        for method_name, attribution_method in attribution_methods.items():
            ...
            try:
                scores = attribution_method(
                    problem, labeled_chunks, base_solution, **plot_kwargs
                )
                ...
                try:
                    ...
                    if method_name == "Activation-Regression":
                        model = train_regression_model(
                            other_problem_dirs, model_type="activation", **kwargs
                        )
                        if model:
                            # Get predictions using this model
                            scores = method_factory(model)(
                                problem, labeled_chunks, base_solution, **plot_kwargs
                            )
                            method_scores[f"{method_name}"] = scores
                    elif method_name == "Metrics-Regression":
                        model = train_regression_model(
                            other_problem_dirs, model_type="metrics", **kwargs
                        )
                        if model:
                            # Get predictions using this model
                            scores = method_factory(model)(
                        ...
                        model = train_regression_model(
                            other_problem_dirs, model_type="metrics_position", **kwargs
                        )
                        if model:
                            # Get predictions using this model
                            scores = method_factory(model)(
                    ...
                    print(
                        f"Error with LOOCV for {method_name} on problem {problem_idx}: {e}"
                    )
                    traceback.print_exc()
        ...
        fig, axes = plt.subplots(
            num_methods + 1, 1, figsize=(12, 3 * (num_methods + 1)), sharex=True
        )
        ...
        for i, (method_name, scores) in enumerate(method_scores.items()):
            if len(scores) != len(true_scores):
                print(
                    f"Warning: {method_name} scores length ({len(scores)}) doesn't match true scores length ({len(true_scores)}) for problem {problem_idx}"
                )
                continue
            ...
            axes[i + 1].plot(
                range(len(scores)), normalized_scores, f"{color}-", linewidth=1.5
            )
        ...
        # Add overall title
        plt.suptitle(
            f"Problem {problem_idx}: Attribution Metrics vs Ground Truth", fontsize=16
        )
    ...
    try:
        ...
        if thinking_model and base_model:
            with torch.no_grad():
                ...
                for pos in range(input_ids.shape[1] - 1):  # Exclude the last token
                    ...
                    kl_div = (
                        (base_probs * (base_log_probs - thinking_log_probs))
                        .sum(dim=-1)
                        .item()
                    )
        ...
        for i, (start_idx, end_idx) in enumerate(chunk_token_ranges):
            ...
            if layer_key in activations:
                ...
                chunk_activations = (
                    layer_activations[0, start_idx:end_idx, :].detach().cpu().numpy()
                )
                ...
                for i in range(len(mean_activations)):  # Use only first 10 dimensions
                    chunk_features[f"mean_activation_dim_{i}"] = float(
                        mean_activations[i]
                    )
...
def train_regression_model(problem_dirs: List[Path], model_type: str, **kwargs) -> Any:
    ...
    for problem_dir in tqdm(problem_dirs, desc=f"Collecting {model_type} features"):
        ...
        # Get features
        features_df = get_chunk_features(
            problem, labeled_chunks, base_solution, **kwargs
        )
        ...
        if model_type == "activation":
            ...
            selected_cols = [
                ...
                if ("activation" in col or "pca_" in col)
            ]
            ...
            selected_cols = [
                col
                for col in features_df.columns
                if ("entropy" in col or "perplexity" in col or "kl" in col)
            ]
...
def activation_regression_attribution(
    problem: Dict, labeled_chunks: List[Dict], base_solution: Dict, **kwargs
) -> List[float]:
    """
    ...
    try:
        ...
        selected_cols = [
            ...
            if (
                "activation" in col
                or "pca_" in col
                or col in ["position", "token_length"]
            )
        ]
        ...
        if len(predicted_scores) < len(labeled_chunks):
            predicted_scores = np.append(
                predicted_scores, [0.5] * (len(labeled_chunks) - len(predicted_scores))
            )
...
def metrics_regression_attribution(
    problem: Dict, labeled_chunks: List[Dict], base_solution: Dict, **kwargs
) -> List[float]:
    """
...
def evaluate_attribution_methods_loocv(
    ...
    **kwargs,
    ...
    for method_name, attribution_method in unsupervised_methods.items():
        ...
        for problem_dir in tqdm(problem_dirs, desc=f"Evaluating {method_name}"):
            ...
            try:
                ...
                print(
                    f"Error evaluating {method_name} on problem {problem_dir.name}: {e}"
                )
                traceback.print_exc()
        ...
        if correlations:
            ...
            print(
                f"{method_name}: Average correlation = {avg_correlation:.4f} ± {std_correlation:.4f} (n={len(correlations)})"
            )
        else:
    ...
    for method_name, method_factory in supervised_methods.items():
        ...
        for test_idx, test_problem_dir in enumerate(
            tqdm(problem_dirs, desc=f"LOOCV for {method_name}")
        ):
            # Create training set (all problems except the test problem)
            train_problem_dirs = [
                dir_path for i, dir_path in enumerate(problem_dirs) if i != test_idx
            ]

            # Train model on training set
            if method_name == "Activation-Regression":
                model = train_regression_model(
                    train_problem_dirs, model_type="activation", **kwargs
                )
            elif method_name == "Metrics-Regression":
                model = train_regression_model(
                    train_problem_dirs, model_type="metrics", **kwargs
                )
            elif method_name == "Activation-Position-Regression":
                model = train_regression_model(
                    train_problem_dirs, model_type="activation_position", **kwargs
                )
            elif method_name == "Metrics-Position-Regression":
                model = train_regression_model(
                    train_problem_dirs, model_type="metrics_position", **kwargs
                )
            elif method_name == "Position-Regression":
                model = train_position_regression_model(train_problem_dirs, **kwargs)
            else:
            ...
            try:
                ...
                scores = attribution_method(
                    test_problem, test_chunks, test_solution, **kwargs
                )
                ...
                print(
                    f"Error evaluating {method_name} on problem {test_problem_dir.name}: {e}"
                )
                traceback.print_exc()
        ...
        if correlations:
            ...
            print(
                f"{method_name} (LOOCV): Average correlation = {avg_correlation:.4f} ± {std_correlation:.4f} (n={len(correlations)})"
            )
        else:
...
def activation_position_regression_attribution(
    problem: Dict, labeled_chunks: List[Dict], base_solution: Dict, **kwargs
) -> List[float]:
    """
    ...
    try:
        ...
        selected_cols = [
            col
            for col in features_df.columns
            if ("activation" in col or "pca_" in col or col == "position")
        ]
        ...
        missing_features = [
            feat for feat in required_features if feat not in features_df.columns
        ]
        if missing_features:
...
def metrics_position_regression_attribution(
    problem: Dict, labeled_chunks: List[Dict], base_solution: Dict, **kwargs
) -> List[float]:
    """
...
def main():
    ...
    problem_dirs = get_problem_dirs(
        analysis_dir, correct_only=args.correct_only, limit=args.max_problems
    )
    ...
    def create_activation_regression(model):
        def method(problem, chunks, solution, **kwargs):
            ...
            return activation_regression_attribution(
                problem, chunks, solution, activation_model=model, **method_kwargs
            )
    ...
    def create_metrics_regression(model):
        def method(problem, chunks, solution, **kwargs):
            ...
            return metrics_regression_attribution(
                problem, chunks, solution, metrics_model=model, **method_kwargs
            )
    ...
    def create_activation_position_regression(model):
        def method(problem, chunks, solution, **kwargs):
            method_kwargs = {
                k: v for k, v in kwargs.items() if k != "activation_position_model"
            }
            return activation_position_regression_attribution(
                problem,
                chunks,
                solution,
                activation_position_model=model,
                **method_kwargs,
            )
    ...
    def create_metrics_position_regression(model):
        def method(problem, chunks, solution, **kwargs):
            method_kwargs = {
                k: v for k, v in kwargs.items() if k != "metrics_position_model"
            }
            return metrics_position_regression_attribution(
                problem, chunks, solution, metrics_position_model=model, **method_kwargs
            )
    ...
    def create_position_regression(model):
        def method(problem, chunks, solution, **kwargs):
            ...
            positions = np.array(
                [i / (num_chunks - 1) for i in range(num_chunks)]
            ).reshape(-1, 1)
    ...
    position_model = train_position_regression_model(
        problem_dirs, use_abs_importance=args.use_abs_importance
    )
    ...
    if activation_model:
        attribution_methods["Activation-Regression (All Data)"] = (
            lambda p, c, s, **kw: activation_regression_attribution(
                p,
                c,
                s,
    ...
    if metrics_model:
        attribution_methods["Metrics-Regression (All Data)"] = (
            lambda p, c, s, **kw: metrics_regression_attribution(
                p,
                c,
                s,
    ...
    if activation_position_model:
        attribution_methods["Activation-Position-Regression (All Data)"] = (
            lambda p, c, s, **kw: activation_position_regression_attribution(
                p,
                c,
                s,
    ...
    if metrics_position_model:
        attribution_methods["Metrics-Position-Regression (All Data)"] = (
            lambda p, c, s, **kw: metrics_position_regression_attribution(
                p,
                c,
                s,
    ...
    if position_model:
        attribution_methods["Position-Regression (All Data)"] = (
            lambda p, c, s, **kw: create_position_regression(position_model)(
                p, c, s, **kw
            )
        )
