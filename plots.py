...
parser.add_argument(
    ...
    dest="normalize",
    action="store_false",
    help="Normalize importance values per problem",
)
parser.add_argument(
    "-nn",
    "--no-normalize",
    dest="normalize",
    action="store_true",
    help="Use raw importance values without normalization",
)
parser.add_argument(
    "-hh",
    "--hierarchical",
    action="store_true",
    default=True,
    help="Use hierarchical statistics (problem-level averages) for category analysis",
)
parser.add_argument(
    "-nhh",
    "--no-hierarchical",
    dest="hierarchical",
    action="store_false",
    help="Use original statistics (chunk-level) for category analysis",
)
parser.add_argument(
    "-ms",
    "--min-steps",
    type=int,
    default=0,
    help="Minimum number of steps (chunks) required for a problem to be included",
)
...
try:
    ...
    print(
        "Warning: scikit-learn not found. Position-debiased plots will not be generated."
    )
...
def collect_tag_data(
    rollouts_dir="math_rollouts/deepseek-r1-distill-qwen-14b/temperature_0.6_top_p_0.95",
):
    """
    ...
    for is_correct in [True, False]:
        ...
        for problem_dir in tqdm(
            problem_dirs,
            desc=f"Processing {'correct' if is_correct else 'incorrect'} problems",
        ):
            ...
            for i in range(
                ...
                if "accuracy" in current_chunk:
                    ...
                    high_convergence = all(
                        "accuracy" in labeled_chunks[i + j]
                        and labeled_chunks[i + j]["accuracy"] > 0.99
                        for j in range(K)
                    )
                    # Check for low accuracy convergence (< 0.01)
                    low_convergence = all(
                        "accuracy" in labeled_chunks[i + j]
                        and labeled_chunks[i + j]["accuracy"] < 0.01
                        for j in range(K)
                    )
            ...
            for chunk in labeled_chunks:
                ...
                if (
                    converged_from_index is not None
                    and chunk_idx >= converged_from_index
                ):
                ...
                record["different_trajectories_fraction"] = (
                ...
                all_metrics_present = all(
                    metric in chunk for metric in IMPORTANCE_METRICS
                )
                if all_metrics_present:
                    for metric in IMPORTANCE_METRICS:
                        record[metric] = (
                            abs(chunk.get(metric)) if chunk.get(metric) > 0.0 else 0.0
                        )
    ...
    print(
        f"Chunks filtered out due to zero different trajectories for counterfactual_importance_accuracy: {filtered_counterfactual_accuracy}"
    )
    print(
        f"Chunks filtered out due to zero different trajectories for counterfactual_importance_kl: {filtered_counterfactual_kl}"
    )

    # Print counts of counterfactual metrics after filtering
    if "counterfactual_importance_accuracy" in df_tags.columns:
        print(
            f"Count of chunks with counterfactual_importance_accuracy after filtering: {df_tags['counterfactual_importance_accuracy'].notna().sum()}"
        )
    if "counterfactual_importance_kl" in df_tags.columns:
        print(
            f"Count of chunks with counterfactual_importance_kl after filtering: {df_tags['counterfactual_importance_kl'].notna().sum()}"
        )
    ...
    for tag, group in tag_groups:
        ...
        if args.hierarchical:
            ...
            if len(problem_position_averages) == 0:
                print(
                    f"Skipping tag {tag} because no problem-level position averages could be computed"
                )
                continue

            pos_mean = np.mean(problem_position_averages)
            pos_std = (
                np.std(problem_position_averages, ddof=1)
                / np.sqrt(len(problem_position_averages))
                if len(problem_position_averages) > 1
                else 0.0
            )
            ...
            for metric in IMPORTANCE_METRICS:
                if metric not in group.columns:
                    print(
                        f"Skipping metric {metric} for tag {tag} because it is not in the dataframe"
                    )
                    continue
                ...
                if len(problem_metric_averages) < 3:
                    print(
                        f"Skipping metric {metric} for tag {tag} because it has less than 3 problem-level averages"
                    )
                    continue
                ...
                metric_std = (
                    np.std(problem_metric_averages, ddof=1)
                    / np.sqrt(len(problem_metric_averages))
                ...
                aggregated_data.append(
                    {
                        ...
                        "total_chunks": len(
                            group
                        ),  # Total number of chunks for reference
                    }
                )
            ...
            for metric in IMPORTANCE_METRICS:
                ...
                if len(valid_values) < 3:
                    print(
                        f"Skipping metric {metric} for tag {tag} because it has less than 3 data points"
                    )
                    continue

                # Calculate statistics
                metric_mean = valid_values.mean()
                metric_std = valid_values.std() / np.sqrt(
                    len(valid_values)
                )  # Standard error
...
def plot_tag_effects(df_agg, output_dir="analysis/importance_plots"):
    ...
    df_agg = df_agg[
        ~df_agg["tag"]
        .apply(lambda x: " ".join(word.capitalize() for word in x.split("_")))
        .isin(categories_to_exclude)
    ]
    ...
    for metric in IMPORTANCE_METRICS:
        ...
        for category in all_categories:
            ...
            category_colors[category] = CATEGORY_COLORS.get(
                category, "#7f7f7f"
            )  # Default to gray
        ...
        for idx, row in df_metric.iterrows():
            ...
            size = (
                100 + 900 * (row["count"] - min_count) / (max_count - min_count)
        ...
        plt.title(
            f"Sentence category effect"
            if args.model == "qwen-14b"
            else f"Sentence category effect\n(R1-Distill-Llama-8B)"
        )
...
def plot_debiased_tag_effects(df_tags, output_dir="analysis/importance_plots"):
    ...
    for metric in IMPORTANCE_METRICS:
        ...
        for tag, group in df_metric.groupby("tag"):
            ...
            if len(group) < 3:
                print(
                    f"Skipping tag {tag} in debiased plot because it has less than 3 data points"
                )
                continue
            ...
            if args.hierarchical:
                ...
                for pn_ci, problem_group in position_problem_groups:
                    if len(problem_group) > 0:
                        problem_avg_position = problem_group[
                            "normalized_position"
                        ].mean()
                        problem_position_averages.append(problem_avg_position)
                ...
                for pn_ci, problem_group in debiased_problem_groups:
                    if len(problem_group) > 0:
                        problem_avg_debiased = problem_group[
                            "debiased_importance"
                        ].mean()
                ...
                # Skip if not enough problem-level averages
                if (
                    len(problem_position_averages) < 3
                    or len(problem_debiased_averages) < 3
                ):
                    print(
                        f"Skipping tag {tag} in debiased plot because it has less than 3 problem-level averages"
                    )
                    continue
                ...
                debiased_std = (
                    np.std(problem_debiased_averages, ddof=1)
                    / np.sqrt(len(problem_debiased_averages))
                    if len(problem_debiased_averages) > 1
                    else 0.0
                )
                ...
                debiased_std = group["debiased_importance"].std() / np.sqrt(
                    count
                )  # Standard error
        ...
        if len(df_debiased) >= 5:
            ...
            correlation = np.corrcoef(
                df_debiased["position_mean"], df_debiased["debiased_mean"]
            )[0, 1]
            ...
            plt.plot(
                ...
                linestyle="--",
                linewidth=2,
                label=f"Best Fit Line (r = {correlation:.2f})",
            )
        ...
        if "accuracy" in metric:
            ...
            if args.hierarchical:
                plt.title(
                    f"Position-Debiased Sentence Category Effect vs. Position (Problem-Level Averages)"
                )
            else:
        ...
        # Save plot
        plt.savefig(
            os.path.join(output_dir, f"{metric}_tag_effects_no_bias.png"), dpi=300
        )
        plt.close()

        # Save the debiased data for further analysis
        df_debiased.to_csv(
            os.path.join(output_dir, f"{metric}_debiased_data.csv"), index=False
        )
        ...
        plt.scatter(
            df_metric["normalized_position"], df_metric[metric], alpha=0.3, s=10
        )
...
def plot_overdeterminedness_comparison(df_tags, output_dir="analysis/importance_plots"):
    ...
    all_categories = sorted(df["formatted_tag"].unique())
    category_colors = {
        cat: CATEGORY_COLORS.get(cat, "#7f7f7f") for cat in all_categories
    }
    ...
    for metric, display_name in metrics_to_plot.items():
        ...
        if HAS_SKLEARN:
            ...
            if len(valid_data) >= 5:
                ...
                # Calculate correlation coefficient
                correlation = np.corrcoef(
                    valid_data["overdeterminedness"], valid_data[plot_metric]
                )[0, 1]
...
def plot_overdeterminedness_category_subplots(
    df_tags, output_dir="analysis/importance_plots"
):
    """
    ...
    metric_groups = {
        "counterfactual": {
            "accuracy": (
                "counterfactual_importance_accuracy",
                "Counterfactual importance (Accuracy)",
            ),
        ...
        "resampling": {
            "accuracy": (
                "resampling_importance_accuracy",
                "Resampling importance (Accuracy)",
            ),
    ...
    norm_status = "normalized" if args.normalize else "raw"
    print(
        f"Generating {norm_status} overdeterminedness category subplot comparisons..."
    )
    ...
    for group_name, metrics in metric_groups.items():
        ...
        for metric_type, (metric_name, _) in metrics.items():
            if metric_name in df.columns and args.normalize:
                df[f"{metric_name}_normalized"] = df.groupby("pn_ci")[
                    metric_name
                ].transform(
                    lambda x: (x - x.min()) / (x.max() - x.min())
                    if x.max() > x.min()
            ...
            for metric_type, (metric_name, display_name) in metrics.items():
                ...
                if HAS_SKLEARN:
                    valid_data = cat_data.dropna(
                        subset=["overdeterminedness", plot_metric]
                    )
                    if len(valid_data) >= 5:
                        x_values = valid_data["overdeterminedness"].values.reshape(
                            -1, 1
                        )
                        y_values = valid_data[plot_metric].values
        ...
        plt.suptitle(
            f"{group_name.capitalize()} Importance vs. Overdeterminedness by Category{norm_suffix}",
            fontsize=16,
            y=1.02,
        )
        plt.tight_layout()
    ...
    for metric, display_name in metrics_to_plot.items():
        ...
        if HAS_SKLEARN:
            ...
            if len(valid_data) >= 5:
                ...
                # Calculate correlation coefficient
                correlation = np.corrcoef(
                    valid_data["normalized_position"], valid_data[metric]
                )[0, 1]
        ...
        # Save plot
        plt.savefig(
            os.path.join(output_dir, f"{metric}_individual_points.png"),
            bbox_inches="tight",
            dpi=300,
        )
        plt.close()
...
def plot_category_metrics_averages(df_tags, output_dir="analysis/importance_plots"):
    ...
    for metric in metrics:
        ...
        grouped = (
            df.groupby("formatted_tag")[metric]
            .agg(["mean", "std", "count"])
            .reset_index()
        )
        ...
        if metric == "different_trajectories_fraction":
            ...
            grouped["abbreviated_tag"] = grouped["formatted_tag"].str.replace(
                "Uncertainty Management", "Uncertainty Mgmt."
            )
            grouped["abbreviated_tag"] = grouped["abbreviated_tag"].str.replace(
                "Result Consolidation", "Result Consol."
            )
        ...
        plt.savefig(
            os.path.join(output_dir, f"avg_{display_metric}_by_category.png"), dpi=300
        )
        plt.close()
...
def plot_kl_metric_comparison_matrix(df_tags, output_dir="analysis/importance_plots"):
    ...
    metric_pairs = [
        (
            "resampling_importance_kl_norm",
            "counterfactual_importance_kl_norm",
        ),  # Resampling vs Counterfactual
        (
            "resampling_importance_kl_norm",
            "forced_importance_kl_norm",
        ),  # Resampling vs Forced
        (
            "counterfactual_importance_kl_norm",
            "forced_importance_kl_norm",
        ),  # Counterfactual vs Forced
    ]
    ...
    fig.suptitle(
        "Comparisons of KL-based Importance Metrics (Normalized 0-1 Scale)", fontsize=16
    )
    ...
    for i, (x_metric, y_metric) in enumerate(metric_pairs):
        ...
        ax.set_title(
            f"{kl_metrics[x_orig].split()[0]} vs {kl_metrics[y_orig].split()[0]}"
        )
...
def plot_kl_metric_comparison_by_category(
    df_tags, output_dir="analysis/importance_plots"
):
    """
    ...
    for metric in kl_metrics.keys():
        if metric in df.columns:
            ...
            df[f"{metric}_norm"] = df[
                metric
            ]  # ((df[metric] - min_val) / (max_val - min_val)).clip(0, 1)
    ...
    for x_metric, y_metric in metric_pairs:
        ...
        fig = plt.figure(figsize=(6 * n_cols, 5 * n_rows))
        fig.suptitle(
            f"{y_display.split()[0]} vs. {x_display.split()[0].lower()} importance by category",
            fontsize=32,
            y=1.00,
        ...
        for idx, category in enumerate(categories, 1):
            ...
            # Filter data for this category
            cat_data = df[df["formatted_tag"] == category].dropna(
            ...
            # Create scatter plot
            ax.scatter(
                cat_data[x_metric], cat_data[y_metric], alpha=0.7, s=30, color=cat_color
            )
            ...
            # Set axis labels
            if (
                "active" in category.lower()
                or "fact" in category.lower()
                or "plan" in category.lower()
                or "result" in category.lower()
                or "uncertainty" in category.lower()
            ):
        ...
        print(
            f"Saved normalized {x_display.split()[0]} vs {y_display.split()[0]} comparison by category to {plot_filename}"
        )

    print("Normalized KL metric comparison plots by category complete.")


def plot_category_vs_overdeterminedness(
    df_tags, output_dir="analysis/importance_plots"
):
    """
...
def plot_category_frequencies(df_tags, output_dir="analysis/importance_plots"):
    ...
    df["formatted_tag"] = df["formatted_tag"].str.replace(
        "Uncertainty Management", "Uncertainty Mgmt."
    )
    ...
    for i, bar in enumerate(bars):
        ...
        color = CATEGORY_COLORS.get(
            category.replace("Uncertainty Mgmt.", "Uncertainty Management"), "#7f7f7f"
...
def filter_problems_by_min_steps(df_tags, min_steps):
    ...
    print(
        f"Omitted {num_omitted} traces with fewer than {min_steps} steps. Remaining: {len(traces_with_enough_steps)} traces."
    )
    return filtered_df
...
def main():
    ...
    # Set paths
    rollouts_dir = (
        f"math_rollouts/deepseek-r1-distill-{args.model}/temperature_0.6_top_p_0.95"
    )
    output_dir = (
        "analysis/importance_plots"
        if args.model == "qwen-14b"
        else "analysis/importance_plots_llama-8b"
    )
