...
parser = argparse.ArgumentParser(
    description="Generate chain-of-thought solutions with rollouts"
)
parser.add_argument(
    "-m",
    "--model",
...
parser.add_argument(
    ...
    "--temperature",
    type=float,
    default=ENDPOINTS.default_temperature,
...
parser.add_argument(
    ...
    "--top_p",
    type=float,
    default=ENDPOINTS.default_top_p,
...
parser.add_argument(
    ...
    "--max_tokens",
    type=int,
    default=ENDPOINTS.default_max_tokens,
...
parser.add_argument(
    "-s", "--seed", type=int, default=44, help="Random seed for reproducibility"
)
parser.add_argument(
    "-f",
    "--force",
    action="store_true",
    help="Force regeneration even if solutions exist",
)
parser.add_argument(
    "-ep",
    "--exclude_problems",
    type=str,
    default=None,
    help="Comma-separated list of problem IDs to exclude",
)
parser.add_argument(
    "-ip",
    "--include_problems",
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Comma-separated list of chunk IDs to include",
)
...
parser.add_argument(
    ...
    type=str,
    default="vLLM",
    choices=["vLLM", "Novita", "Together", "Fireworks", "Local"],
...
parser.add_argument(
    "-tk", "--top_k", type=int, default=ENDPOINTS.default_top_k, help="Top-k parameter"
)
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="Use quantization for local model",
)
parser.add_argument(
    "-bs", "--batch_size", type=int, default=8, help="Batch size for local model"
)
parser.add_argument(
    "-mr",
    "--max_retries",
...
if args.provider == "Local":
    try:
        ...
        )  # Slight adjustment we need to make
...
def generate_with_local_model_batch(
    prompts: List[str], temperature: float, top_p: float, max_tokens: int
) -> List[Dict]:
        ...
        for i in range(0, len(prompts), batch_size):
            ...
            with torch.no_grad():
                batch_outputs = local_model.generate(
                    **batch_inputs, **generation_config
                )

            # Process each output in the batch
            for j, (input_ids, output_ids) in enumerate(
                zip(batch_inputs["input_ids"], batch_outputs)
            ):
...
async def make_api_request(
    ...
    *,
    ...
    if (
        args.min_p is not None and args.provider != "Fireworks"
    ):  # Fireworks doesn't support min_p
    ...
    for attempt in range(max_retries):
        try:
            ...
            if (
                args.provider == "Together" or args.provider == "Fireworks"
...
async def process_problem(problem_idx: int, problem: Dict) -> None:
    ...
    if base_solution is None:
        ...
        if (
            ...
            print(base_solution["solution"])
            print(
                f"Problem {problem_idx}: Base solution is CORRECT or has error. Retrying..."
            )
            return await process_problem(problem_idx, problem)
    ...
    for chunk_idx, (chunk, full_prefix) in enumerate(zip(chunks, cumulative_chunks)):
        if args.include_chunks and str(chunk_idx) not in args.include_chunks.split(","):
            print(
                f"Problem {problem_idx}, Chunk {chunk_idx}: Skipping (not in include_chunks)"
            )
            continue
        ...
        if solutions_file.exists() and not args.force:
            with open(solutions_file, "r", encoding="utf-8") as f:
                ...
                if not args.skip_recalculate:
                    ...
                    for rollout in existing_solutions:
                        if "rollout" in rollout and "error" not in rollout:
                            ...
                            # Update if different
                            if (
                                rollout.get("answer") != answer
                                or rollout.get("is_correct") != is_correct
                            ):
                                updated_count += 1
                    ...
                    if updated_count > 0:
                        print(
                            f"Problem {problem_idx}, Chunk {chunk_idx}: Updated accuracy for {updated_count} rollouts"
                        )
                ...
                print(
                    f"Problem {problem_idx}, Chunk {chunk_idx}: Found {len(valid_existing_solutions)} valid solutions"
                )
        ...
        if num_rollouts_needed > 0:
            print(
                f"Problem {problem_idx}, Chunk {chunk_idx}: Generating {num_rollouts_needed} rollouts"
            )
            ...
            if args.provider == "Local":
                ...
                for i, result in enumerate(batch_results):
                    ...
                    prefix_without_chunk = full_prefix.replace(chunk, "").strip()
                    chunk_resampled = (
                        split_solution_into_chunks(rollout_text)[0]
                        if rollout_text
                        else ""
                    )
            ...
            print(
                f"Problem {problem_idx}, Chunk {chunk_idx}: Saved {len(all_solutions)} solutions"
            )
        else:
            print(
                f"Problem {problem_idx}, Chunk {chunk_idx}: Already have {len(valid_existing_solutions)} valid solutions"
            )
